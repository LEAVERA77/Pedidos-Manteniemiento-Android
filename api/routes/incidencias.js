...
async function tenantEsCooperativaElectrica(tenantId) {
  try {
    const r = await query(`SELECT lower(trim(coalesce(tipo, ''))) AS t FROM clientes WHERE id = $1 LIMIT 1`, [
      tenantId,
    ]);
    const t = String(r.rows[0]?.t || "");
...
/**
 * GET /api/incidencias/sugerir-por-infra?pedido_id=
 * Cooperativa eléctrica: candidatos abiertos con mismo trafo o distribuidor (sin incidencia previa).
 */
