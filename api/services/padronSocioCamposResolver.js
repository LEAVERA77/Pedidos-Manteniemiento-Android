/**
 * Dist., Conex. y Fases desde socios_catalogo (columnas + datos_extra).
 * made by leavera77
 */
...
export function parseDatosExtraSocio(raw) {
  ...
  try {
    ...
    if (j && typeof j === "object" && !Array.isArray(j)) return parseDatosExtraSocio(j);
  } catch (_) {}
  return {};
}
...
export function resolverTipoConexionSocio(row) {
  return pickCampo(row, ["tipo_conexion", "conexion", "Conex", "conex", "tipo_de_conexion"]);
}
