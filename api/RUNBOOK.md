...
- Variable **`WHATSAPP_HUMAN_CHAT_DIRECT_PHONES`**: teléfonos (solo dígitos; varios separados por coma o `;`, misma normalización que *activar* / *desactivar*). **Hay que definirla en Render** (sin variable = desactivado). Esos remitentes abren el chat humano con el **primer mensaje** (no hace falta *menú* → *Otros*). *No* uses un número *maestro* (`WHATSAPP_BOT_MASTER_PHONE*`) ni el de un usuario admin: esos se excluyen para no romper pruebas del bot. *Menú* / *0* / *inicio* / *ayuda* siguen yendo al resumen. Vacío, `0`, `false` u `off` desactiva la lista.
