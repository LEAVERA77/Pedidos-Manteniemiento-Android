 ...
 */
function seleccionarDistribuidorPorCodigo(cod, di2) {
    if (!di2 || !cod) return;
    const c = String(cod).trim().toUpperCase();
    const opt = Array.from(di2.options).find((o) => (o.value || '').trim().toUpperCase() === c);
...
export function aplicarPadronAlFormularioNuevoPedido(row, opts = {}) {
    ...
    if (opts.esCooperativaElectrica) {
        ...
        if (row.distribuidor_codigo) seleccionarDistribuidorPorCodigo(row.distribuidor_codigo, di2);
        if (refEl && row.barrio != null) refEl.value = String(row.barrio).trim();
