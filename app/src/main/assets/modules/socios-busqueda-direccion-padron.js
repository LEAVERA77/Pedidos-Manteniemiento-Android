/**
 * Búsqueda fuzzy por dirección en padrón: socios_catalogo (todos los rubros) + clientes_finales (municipio/agua).
 * made by leavera77
 */

import { direccionCoincideFuzzy } from './gn-fuzzy-texto-levenshtein.js';
 ...
 * }} deps
 * @param {string} raw calle o domicilio
 * @returns {Promise<Array<Record<string, unknown> & { padronSource: PadronSource }>>}
 */
export async function buscarPersonasPadronPorDireccionFuzzy(deps, raw) {
    ...
    try {
        const r = await deps.sqlSimple(
            `SELECT s.id, s.nis_medidor, s.nis, s.medidor, s.nombre, s.calle, s.numero, s.barrio, s.telefono, s.localidad, s.provincia,
             ...
             ${wSoc}
             ORDER BY s.calle NULLS LAST, s.nombre NULLS LAST
             LIMIT ${LIM}`
        );
        for (const row of r.rows || []) {
            if (!direccionCoincideFuzzy(needle, row)) continue;
        ...
        try {
            const rCf = await deps.sqlSimple(
                `SELECT id, nombre, apellido, nis, medidor, numero_cliente, calle, numero_puerta, localidad, barrio, telefono
                 ...
                 ORDER BY calle NULLS LAST, apellido NULLS LAST
                 LIMIT ${LIM}`
            );
            for (const row of rCf.rows || []) {
                ...
                if (!direccionCoincideFuzzy(needle, dom)) continue;
                push({
                    id: row.id,
                    nombre: nom,
